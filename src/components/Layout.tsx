...
export default function Layout() {
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      if (!user) return;
      if (user.email === 'nithinkumarkatta90@gmail.com') { setIsAdmin(true); return; }
      try {
      ...
      <Link
        ...
        className={`group relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
          isActive
            ...
            : isDark ? 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'
        }`}
      >
        {isActive && (
          <motion.span
        ...
        <item.icon className={`w-4 h-4 flex-shrink-0 transition-colors ${
          isActive ? (isDark ? 'text-teal-400' : 'text-teal-600') : (isDark ? 'text-slate-500 group-hover:text-slate-300' : 'text-gray-400 group-hover:text-gray-600')
        }`} />
        <span className="truncate">{item.name}</span>
        {isActive && <ChevronRight className={`ml-auto w-3.5 h-3.5 flex-shrink-0 ${isDark ? 'text-teal-500/60' : 'text-teal-400'}`} />}
      </Link>
    );
  };

  const NavContent = ({ onNavigate }: { onNavigate?: () => void }) => (
    <div className={`flex flex-col h-full transition-colors ${isDark ? 'bg-[#0F172A] border-r border-slate-800/60' : 'bg-white border-r border-gray-200'}`}>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-5">
        ...
        {isAdmin && (
          <div>
            <p className={`px-3 mb-1.5 text-[10px] font-bold uppercase tracking-widest ${isDark ? 'text-teal-600' : 'text-teal-500'}`}>Admin</p>
      ...
      <div className={`p-3 border-t ${isDark ? 'border-slate-800/60' : 'border-gray-200'}`}>
        <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl mb-1 ${isDark ? 'bg-slate-800/50' : 'bg-gray-50'}`}>
          ...
          <div className="flex-1 min-w-0">
            <p className={`text-xs font-semibold truncate ${isDark ? 'text-slate-200' : 'text-gray-800'}`}>{user?.displayName || 'User'}</p>
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all ${
            isDark ? 'text-slate-500 hover:text-rose-400 hover:bg-rose-500/10' : 'text-gray-400 hover:text-rose-600 hover:bg-rose-50'
          }`}
        >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header
          className={`h-16 flex items-center justify-between px-4 sm:px-6 border-b flex-shrink-0 backdrop-blur-md transition-colors ${
            isDark ? 'bg-[#0F172A]/80 border-slate-800/60' : 'bg-white/80 border-gray-200'
          }`}
        ...
        >
          <div className="flex items-center gap-4 flex-1">
            <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
              <SheetTrigger
                render={
                  <button className={`lg:hidden p-2 rounded-xl transition-colors ${isDark ? 'text-slate-400 hover:bg-slate-800 hover:text-white' : 'text-gray-500 hover:bg-gray-100'}`}>
            ...
            <span className={`text-sm font-semibold lg:hidden ${isDark ? 'text-white' : 'text-gray-900'}`}>{currentPageName}</span>

            <div className={`hidden md:flex items-center gap-2 rounded-xl px-3 py-2 w-72 border text-sm transition-colors ${
              isDark ? 'bg-slate-800/60 border-slate-700/50 focus-within:border-teal-500/50 focus-within:bg-slate-800' : 'bg-gray-50 border-gray-200 focus-within:border-teal-400'
            }`}>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-2 rounded-xl transition-all ${isDark ? 'text-slate-400 hover:bg-slate-800 hover:text-yellow-400' : 'text-gray-500 hover:bg-gray-100 hover:text-yellow-500'}`}
            >
              {isDark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </button>

            <Link to="/reminders" className={`relative p-2 rounded-xl transition-all ${isDark ? 'text-slate-400 hover:bg-slate-800 hover:text-teal-400' : 'text-gray-500 hover:bg-gray-100'}`}>
              <Bell className="w-4 h-4" />
            </Link>

            <Link to="/profile">
              <Avatar className={`h-8 w-8 ring-2 transition-all cursor-pointer ${isDark ? 'ring-slate-700 hover:ring-teal-500/60' : 'ring-gray-200 hover:ring-teal-400'}`}>
                <AvatarImage src={user?.photoURL || ''} />
                <AvatarFallback className={`text-xs font-bold ${isDark ? 'bg-teal-900 text-teal-300' : 'bg-teal-100 text-teal-700'}`}>
                  {user?.displayName?.charAt(0) || 'U'}
                </AvatarFallback>
            ...
            return (
              <Link
                ...
                className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all relative ${
                  isActive ? (isDark ? 'text-teal-400' : 'text-teal-600') : (isDark ? 'text-slate-500' : 'text-gray-400')
                }`}
              >
                {isActive && (
                  <span className={`absolute -top-px left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full ${isDark ? 'bg-teal-400' : 'bg-teal-600'}`} />
                )}
