...
import { completeGoogleRedirectSignIn, getAuthErrorMessage, signInWithGoogle } from '../lib/auth';
  ...
  useEffect(() => {
    const finishRedirectSignIn = async () => {
      ...
      try {
        ...
        toast.error(getAuthErrorMessage(error, 'Failed to login with Google'));
      } finally {
        ...
        <CardHeader className="space-y-4 text-center pt-10">
          ...
          <div>
            ...
            <CardDescription className="text-base mt-2 font-medium">
              Sign in to continue to Pharma Care AI
            </CardDescription>
          </div>
        </CardHeader>
